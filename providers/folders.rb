action :backup do
	cron "scheduled backup: " + new_resource.name do
		...
		command "#{new_resource.base_dir}/folders_backup_#{new_resource.name}.sh "
		path "/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin:#{new_resource.base_dir}"
		action :create
	end
	template "#{new_resource.base_dir}/folders_backup_#{new_resource.name}.sh" do
		mode 0700
		owner "sa-backup"
		group "root"
		source new_resource.options["source"] || "folders_backup.sh.erb"
...
action :remove do
	file "#{new_resource.base_dir}/folders_backup_ #{new_resource.name}.sh " do
		action :delete
	end
	cron "scheduled backup: " + new_resource.name do
		hour new_resource.hour || "1" 
